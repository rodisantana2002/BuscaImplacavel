        ...
        with open(self.destino, 'a+') as source:
            ...
            with open(arquivo, 'r') as base:
                ...
                logger.debug(
                    '+----------------------------------------------------------+')
                logger.debug(
                    '|     Inciando script de conversão de arquivos (JabRef)    |')
                logger.debug(
                    '+----------------------------------------------------------+')
                count = 1
                for row in reader:
                    if count >= self.limiteInf and count <= self.limiteSup:
                        if row['DOI'] != '':
                            ...
                            strLog = '---> {0} - {1}'.format(
                                row['Identifier'], row['DOI'])
                            logger.debug(strLog)
                        else:
                            strLog = '---> [erro] {0} - artigo nao possui DOI, ele foi removido da conversao'.format(
                                row['Identifier'])
                            logger.debug(strLog)
                    count += 1
        ...
        with open(self.destino, 'a+') as source:
            ...
            with open(arquivo, 'r') as base:
                ...
                logger.debug(
                    '+----------------------------------------------------------+')
                logger.debug(
                    '|      Inciando script de conversão de arquivos (IEEE)     |')
                logger.debug(
                    '+----------------------------------------------------------+')
                count = 1
                for row in reader:
        ...
        with open(self.destino, 'a+') as source:
            ...
            with open(arquivo, 'r') as base:
                ...
                logger.debug(
                    '+----------------------------------------------------------+')
                logger.debug(
                    '|    Inciando script de conversão de arquivos (Springer)   |')
                logger.debug(
                    '+----------------------------------------------------------+')
                count = 1
                for row in reader:
                    if count >= self.limiteInf and count <= self.limiteSup:
                        if row['Item DOI'] != '':
                            ...
                            strLog = '---> {0} - {1}'.format(
                                row['Publication Year'], row['Item DOI'])
                            logger.debug(strLog)
                        else:
                            strLog = '---> [erro] {0} - artigo nao possui DOI, ele foi removido da conversao'.format(
        ...
        with open(self.destino, 'a+') as source:
            ...
            with open(arquivo, 'r') as base:
                ...
                logger.debug(
                    '+----------------------------------------------------------+')
                logger.debug(
                    '|     Inciando script de conversão de arquivos (ACM)       |')
                logger.debug(
                    '+----------------------------------------------------------+')
                count = 1
                for row in reader:
        ...
        with open(self.destino, 'a+') as source:
            ...
            with open(arquivo, 'r') as base:
                ...
                logger.debug(
                    '+----------------------------------------------------------+')
                logger.debug(
                    '|    Inciando script de conversão de arquivos (Science)    |')
                logger.debug(
                    '+----------------------------------------------------------+')
                count = 1
                for row in reader:
